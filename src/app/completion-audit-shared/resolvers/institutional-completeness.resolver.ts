...
import { getInstitutionalCompleteness } from '../store/actions/completion.actions';
import { CompletionStoreType } from '../store/reducers';

@Injectable()
export class InstitutionalCompletenessResolver implements Resolve<boolean> {
  ...
  resolve(): Observable<boolean> {
    this.store.select(selectPropertyDetails).pipe(
      ...
      this.store.dispatch(getInstitutionalCompleteness({ propertyId }));
    });
    return of(true);
  }
}
